...
{
	/// <summary>
	///		A fox data source stored in a Microsoft Access database.
	...
	///		method.
	/// </summary>
	public class cMDBDataSource : cFoxDataSource
	{
		/// <summary>
		///		Initialize a fox Access datasource.  The Connect method must be called to
        ...
        public cMDBDataSource(string EmptyPath, bool IsUnix) 
			: base(IsUnix)
		{
		...
		// ********************************* Properties *************************************
		/// <summary>
		///		The path to the empty database used to create new fox datasources
		///		(read-only).
		/// </summary>
		...
		/// <summary>
		///		Disconnect from the fox datasource and close the database.
		/// </summary>
		public override void Disconnect()
		{
			if (mvarConnection != null) {
				if (mvarConnection.State > 0) {
					...
					mvarDataSet.Clear();
					// close the database if required
                    try
                    {
													...
													cBackground Background) 
		{
            ...
            if (dt.Columns.Contains("ListIndex")) Attributes.ListIndex = Convert.ToInt32(dr["ListIndex"]);
			try 
            {
