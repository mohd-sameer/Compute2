...
namespace Raccoon
{
	...
	/// <summary>
	///		Summary description for cXMLRaccoonDataSource.
	/// </summary>
	public class cXMLRaccoonDataSource : cRaccoonDataSource
	{
		/// <summary>
		///		Initialize an XML based raccoon data source.  Fieldnames are long by default.
		...
		public cXMLRaccoonDataSource(bool IsUnix) : base(IsUnix)
		{
			mvarFieldSize = enumFieldNameSize.LongNames;
		}

		/// <summary>
		///		Initialize an XML based raccoon data source, specifying the field name size.
        ...
        public cXMLRaccoonDataSource(enumFieldNameSize FieldNameSize, bool IsUnix)
            : base(IsUnix)
		{
		...
		/// <summary>
		///		Disconnect from the raccoon datasource.
		/// </summary>
		...
		protected override void WriteAnimalRecord(cAnimalAttributes Attributes, bool DeleteFirst)
		{
			...
			foreach (cInfection Inf in Attributes.Infections) 
			{
				...
				dr[FName["Year"]] = Inf.Year;
				dr[FName["Week"]] = Inf.Week;
				dr[FName["IncubationPeriod"]] = Inf.IncubationPeriod;
				dr[FName["InfectiousPeriod"]] = Inf.InfectiousPeriod;
				dr[FName["InfectingAnimalID"]] = Inf.InfectingAnimalID;
				dt.Rows.Add(dr);
			}
			...
			foreach (cVaccine Vaccine in Attributes.Vaccines) 
			{
				...
				dr[FName["Year"]] = Vaccine.Year;
				dr[FName["Week"]] = Vaccine.Week;
		...
		protected override bool GetNextAnimalRecord(cAnimalAttributes Attributes, cBackground Background) 
		{
            ...
            for (; DiseasePosition < mvarDataSet.Tables[TName["Disease"]].Rows.Count; DiseasePosition++)
            {
                ...
                int Year = (int)drChild[FName["Year"]];
                int Week = (int)drChild[FName["Week"]];
        ...
        private void CreateDataViewSorts()
        {
            ...
            mvarDataSet.Tables[TName["Disease"]].DefaultView.Sort = string.Format("{0},{1},{2}", FName["Animal_ID"], FName["Year"], FName["Week"]);
            mvarDataSet.Tables[TName["Vaccines"]].DefaultView.Sort = string.Format("{0},{1},{2}", FName["Animal_ID"], FName["Year"], FName["Week"]);
        }
		...
		private string[,] FieldNames = new string[44, 2] {
															{"ID", "A"},
															{"Age", "B"},
															{"Parent_ID", "C"},
															{"Gender", "D"},
